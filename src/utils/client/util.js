...
export const showImageUrl = (imageData) => {
    ...
    if (typeof imageData === "object" && imageData?.url) {
        return imageData.url.startsWith("/") ? baseUrl + imageData.url.slice(1).replaceAll('\\', "/") : baseUrl + imageData.url.replaceAll('\\', "/");
    }
    ...
    if (imageData.startsWith("/")) return baseUrl + imageData.slice(1).replaceAll('\\', "/");
    ...
    try {
        ...
        if (parsed?.url) {
            return parsed.url.startsWith("/")
                ? baseUrl + parsed.url.slice(1).replaceAll('\\', "/")
                : baseUrl + parsed.url.replaceAll('\\', "/");
        }
    } catch {
