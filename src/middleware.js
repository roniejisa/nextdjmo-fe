...
async function authenticate(request, token = null, refreshToken = null, isRefresh = false, isOauth = false) {
    ...
    if (token) {
        ...
        if (cached && cached.expiresAt > now) { // Cache valid if more than 10s remaining
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (verifyResult && verifyResult.status === 200 && verifyResult.data && verifyResult.data.valid) {
                    // Cache the result
                    ...
                    if (tokenCache.size > 1000) { // Prevent memory leak
                        cleanupCache()
                    }
...
function setResponse(userId, username, accessToken, refreshToken, request, isAuthenticated, isSocial, newCustomer, shouldClearTokens = false) {
...
export async function middleware(request) {
    ...
    try {
        ...
        const { isAuthenticated, userId, username, accessToken, refreshToken, isSocial, shouldClearTokens } = authResult
        ...
        const response = setResponse(userId, username, accessToken, refreshToken, request, isAuthenticated, isSocial, newCustomer, shouldClearTokens)
