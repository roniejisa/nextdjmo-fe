...
async function authenticate(request, token = null, refreshToken = null, isRefresh = false) {
    ...
    if (token) {
        ...
        if (profile.status == 200) {
            return { isAuthenticated: true, user: profile.data, accessToken: token, refreshToken }
        }
    }
...
function setResponse(user, accessToken, refreshToken, request) {
    ...
    if (accessToken) response.cookies.set('token', accessToken, {
        httpOnly: true,
        secure: true,
        path: "/",
        sameSite: "strict"
...
export async function middleware(request) {
    ...
    const { isAuthenticated, accessToken, refreshToken, user } = await authenticate(request)
    ...
    return setResponse(user, accessToken, refreshToken, request)
}
