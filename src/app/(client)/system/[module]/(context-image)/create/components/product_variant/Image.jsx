...
const ImageComponent = ({ defaultValue, fnChooseImage, attrName, attrValue }) => {
  ...
  useEffect(() => {
    ...
    if (index !== -1) {
      imageRef.current.src = itemCurrent[index]?.data
        ...
        : "/next.svg";
      fnChooseImage(JSON.stringify(itemCurrent[index]?.data), attrName, attrValue); 
      setShowMedia(false);
    }
  ...
  return (
    <div className="relative">
      <div className="group w-[60px] h-[60px] cursor-pointer group" onClick={handleShowUpload}>
        <Image
          ref={imageRef}
          src={
