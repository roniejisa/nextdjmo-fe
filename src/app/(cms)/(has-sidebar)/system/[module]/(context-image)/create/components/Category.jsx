...
// Utility function - không thay đổi logic
...
const ToggleButton = ({ hasChildren, onClick }) => {
...
const CategoryItem = ({
  ...
  const handleToggle = (e) => {
    ...
    const catalogChild = button.closest("label").nextElementSibling;
    ...
    <li className={`${level > 0 ? "ml-6" : ""} mb-2`}>
      <label className="cursor-pointer block group">
        ...
        >
          ...
          {/* Toggle Button */}
          <ToggleButton hasChildren={hasChildren} onClick={handleToggle} />
        </div>
          ...
          <ul className="space-y-1">
            {options.map((option) => (
              <CategoryItem
                ...
                onToggleChildren={handleShowChildren}
              />
            ))}
          </ul>
        </div>
