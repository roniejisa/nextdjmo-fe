...
import { generateCombinations, generateKey, sortData } from "@/utils/client";
...
const ProductVariant = ({ field, item, value }) => {
  ...
  useEffect(() => {
    try {
      itemRef.current = JSON.parse(value);
    } catch (e) {}
    if (
      itemRef.current &&
      "data" in itemRef.current &&
      "listAttribute" in itemRef.current &&
      itemRef.current.listAttribute.length > 0
    ) {
      setHasVariant(true);
      setListAttribute(itemRef.current.listAttribute);
      setProductAttributes(
        field.data_product_attributes.filter(
          (item) =>
            !itemRef.current.listAttribute.some(
              (attribute) => attribute.name == item.name
            )
        )
      );
    } else if (itemRef.current && "data" in itemRef.current) {
      setData([...itemRef.current.data]);
    }
  ...
  useEffect(() => {
    if (listAttribute.length > 0) {
      ...
      setData((prev) => {
        if (prev.length === 0 && itemRef.current) {
          prev = itemRef.current.data;
        }
        const newListData = listData.map((data) => {
          const key = generateKey(data);
          const item = prev.find((prevItem) => {
