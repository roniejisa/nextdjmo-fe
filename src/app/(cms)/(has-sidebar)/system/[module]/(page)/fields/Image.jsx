...
const ImageComponent = ({ value, item, field }) => {
  ...
  useEffect(() => {
    ...
    if (parsedValue) {
      ...
      if (!existingItem || JSON.stringify(existingItem.data) !== JSON.stringify(parsedValue)) {
        setItemCurrent((prev) => {
      ...
      {hasImage && currentImage?.data && currentImage?.data?.file_info && (
        <div className="mt-2 px-3 py-2 bg-gray-50 rounded-lg border border-gray-100">
          <div className="text-xs text-gray-600">
            <div className="font-medium truncate mb-0.5 text-gray-800" title={currentImage.data.filename + currentImage.data.extention || 'image.jpg'}>
              {currentImage.data.filename + currentImage.data.extention || 'image.jpg'}
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-500">
                {isJSON(currentImage?.data?.file_info)
                  ? `${(JSON.parse(currentImage?.data?.file_info)?.['size'] / 1024).toFixed(1)} KB`
                  : 'Size unknown'
                }
              </span>
