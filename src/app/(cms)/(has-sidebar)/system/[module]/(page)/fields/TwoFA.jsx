...
import { confirmOtp, disabledOtp, enableOtp } from "./action";
import Image from "next/image";

const TwoFA = ({ value, item }) => {
  const { setShowModalQuestion, setModalOptions, profile } = useContext(CMSContext);
  const [on2Fa, setOn2Fa] = useState(value);
  ...
  // Đây là chế độ thêm
  if(!item){
    return <>Chức năng này chỉ thực hiện sau khi đã tạo tài khoản</>
  }
