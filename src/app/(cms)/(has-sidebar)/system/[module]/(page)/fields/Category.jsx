...
const CategoryModal = ({
  ...
  const { data: childrenData, isLoading: childrenLoading } = useApiCall(
    currentParentId && !isSearchMode
      ? `children-${currentParentId}-${currentPage}`
      : null,
  ...
  const { categories, totalPages, total } = useMemo(() => {
    ...
    const items =
      responseData.items || responseData.data?.items || responseData || [];
    const totalCount =
      responseData.total || responseData.data?.total || items.length;
  ...
  const isParentOfSelected = (categoryId) => {
    return selectedCategories.some((selected) => {
      return (
        selected.path &&
        selected.path.some((pathItem) => pathItem.id === categoryId)
      );
    });
  };
  ...
  const getAllChildrenIds = (categoryId, allCategories = categories) => {
    const children = allCategories.filter(
      (cat) => cat[field.module_parent_id] === categoryId
    );
    ...
    children.forEach((child) => {
      if (child.has_category === "1") {
        childrenIds = [
          ...childrenIds,
          ...getAllChildrenIds(child[field.module_id], allCategories),
        ];
      }
    });
  ...
  const handleCheckboxChange = (category, isChecked) => {
    ...
    if (isChecked) {
      ...
      const childrenIds =
        category.has_category === "1" ? getAllChildrenIds(categoryId) : [];
        ...
        <div className="bg-gradient-to-r from-gray-50/80 to-gray-100/80 backdrop-blur-md border-t border-white/20 p-6">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-600">
              ...
              danh mục
              {isSearchMode && (
                <span className="ml-2 text-orange-600">(Chế độ tìm kiếm)</span>
              )}
            </p>
            <button
              type="button"
...
const Category = ({
  ...
  const handleCategoryRemove = (categoryId) => {
    setSelectedCategories((prev) => {
      // Tìm category bị xóa
      const categoryToRemove = prev.find(
        (cat) => cat[field.module_id] === categoryId
      );
      ...
      if (categoryToRemove) {
        ...
        return prev.filter((cat) => {
          ...
          if (cat.path && categoryToRemove.path) {
            const isChild = cat.path.some(
              (pathItem) => pathItem.id === categoryToRemove[field.module_id]
            );
            return !isChild;
          }
