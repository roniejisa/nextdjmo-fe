...
const ImageComponent = ({ defaultValue, item, field }) => {
  ...
  // Helper function để update UI
  const updateUI = (imageData) => {
    ...
    if (inputRef.current) {
      inputRef.current.value = imageData ? JSON.stringify(imageData) : "";
    }
  ...
  return (
    <div className="relative">
      ...
      {/* Compact File Info */}
      {hasImage && currentImage?.data?.file_info && (
        <div className="mt-2 px-3 py-2 bg-gray-50 rounded-lg border border-gray-100">
          <div className="text-xs text-gray-600">
            <div
              ...
              title={
                (currentImage.data.filename || "image") + 
                (currentImage.data.extention || ".jpg")
              }
            >
              {(currentImage.data.filename || "image") + 
               (currentImage.data.extention || ".jpg")}
