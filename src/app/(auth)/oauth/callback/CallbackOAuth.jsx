...
const CallbackOAuth = ({ searchParams }) => {
  const [status, setStatus] = useState("processing"); // processing, success, error, otp
  ...
  const saveToken = async () => {
    try {
      ...
      const ssid = searchParams.ssid;
      if (ssid) {
        setStatus("otp"); // Set trạng thái OTP
        channel.postMessage({
          type: "login-otp",
          _id: ssid,
        });

        // Đóng cửa sổ sau 2 giây để người dùng thấy thông báo
        setTimeout(() => {
          window.close();
        }, 2000);
        return true;
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full mx-4">
        ...
        {status === "otp" && (
          <div className="text-center">
            ...
            <p className="text-gray-600">Quay lại trang chính để nhập mã xác thực</p>
          </div>
        )}
